...
func RunTestCommon(t *testing.T, kv store.FlightKv) {
...
// backends
func RunTestAtomic(t *testing.T, kv store.FlightKv) {
...
// by the K/V backends.
func RunTestWatch(t *testing.T, kv store.FlightKv) {
...
// by the K/V backends.
func RunTestLock(t *testing.T, kv store.FlightKv) {
...
// by the K/V backends.
func RunTestLockTTL(t *testing.T, kv store.FlightKv, backup store.FlightKv) {
...
// available.
func RunTestLockWait(t *testing.T, kv store.FlightKv, backup store.FlightKv) {
...
func RunTestTTL(t *testing.T, kv store.FlightKv, backup store.FlightKv) {
	t.Run("PutTTL", func(t *testing.T) {
		testPutTTL(t, kv, backup)
	})
}

func testPutGetDeleteExists(t *testing.T, kv store.FlightKv) {
...
func testWatchTree(t *testing.T, kv store.FlightKv) {
	dir := "testWatchTree"
...
func testAtomicPut(t *testing.T, kv store.FlightKv) {
	key := "testAtomicPut"
	value := []byte("world")
	...
	pair.LastIndex = 6744
	success, pair2, err := kv.AtomicPut(key, []byte("WORLDWORLD"), pair, nil)
	assert.Error(t, err, pair2)
	assert.False(t, success, pair2)
}

func testAtomicPutCreate(t *testing.T, kv store.FlightKv) {
...
func testAtomicPutWithSlashSuffixKey(t *testing.T, kv store.FlightKv) {
...
func testAtomicDelete(t *testing.T, kv store.FlightKv) {
...
func testLockUnlock(t *testing.T, kv store.FlightKv) {
	key := "testLockUnlock"
	value := []byte("bar")
...
func testLockTTL(t *testing.T, kv store.FlightKv, otherConn store.FlightKv) {
	key := "testLockTTL"
	value := []byte("bar")
...
func testLockWait(t *testing.T, kv, otherConn store.FlightKv) {
	key := "testLockWait"
...
func testPutTTL(t *testing.T, kv store.FlightKv, otherConn store.FlightKv) {
	firstKey := "testPutTTL"
	firstValue := []byte("foo")
...
func testList(t *testing.T, kv store.FlightKv) {
	prefix := "testList"
...
func testDeleteTree(t *testing.T, kv store.FlightKv) {
...
func RunCleanup(t *testing.T, kv store.FlightKv) {
	for _, key := range []string{
